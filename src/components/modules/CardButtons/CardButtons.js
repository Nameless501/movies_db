...
function CardButtons({ place, id, type }) {
  ...
  function handleAddToWatchlist(evt) {
    ...
    if (isInWatchlist) {
      openConstructionPopup(
        "В настоящий момент API TMDB не поддерживает удаление элементов из пользовательских списков. Как только эта возможность будет добавлена, то она будет реализована и на этой странице. Пока это можно сделать на сайте TMDB в своем профиле."
      );
    } else {
  ...
  return (
    <div
      ...
      ref={buttonsRef}
    >
      <ButtonTooltip text="Смотреть позже" place={place}>
        <IconButton
          Icon={
            states?.[type]?.[id]?.watchlist
              ? BsBookmarkStarFill
              : BsBookmarkPlus
          }
      ...
      <ButtonTooltip text="Поделиться" place={place}>
        <IconButton
          Icon={HiShare}
          place={place}
      ...
      <ButtonTooltip text="Трейлер" place={place}>
        <IconButton
      ...
      <ButtonTooltip text="Оценить" place={place}>
        <IconButton
