...
function RatingPopup() {
  ...
  const state = useSelector(
    (state) => state.user?.states?.[data.type]?.[data.id]
  );
  ...
  function handleRatingSet() {
    dispatch(setRating({ ...data, value })).then(() => closeAll());
  }
  ...
  return (
    <PopupWrapper handleClose={closeAll}>
      <div className="rating-popup">
        ...
        <CloseButton place="popup" handleClick={closeAll} />
      </div>
    </PopupWrapper>
  );
}
